...
# Example usage: bash mult.sh

# Loop over a sequence of values for the dihedral angle from -3.1 radians to 1.5 radians with a step of 0.05
for AT in $(seq -3.1 0.05 1.5)
do
...
gmx_mpi mdrun -plumed eq1_plumed.dat -deffnm eq1_nvt$AT -x eq1_traj$AT.xtc
...
# Run the simulation with GROMACS mdrun
gmx_mpi mdrun -plumed eq2_plumed.dat -deffnm eq2_nvt$AT -x eq2_traj$AT.xtc
...
# Run the simulation with GROMACS mdrun
mpirun -np 1 gmx_mpi mdrun -plumed plumed.dat -deffnm us$AT -x traj$AT.xtc
