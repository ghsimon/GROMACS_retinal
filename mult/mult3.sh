...
# Example usage: bash mult3.sh

# Loop over a sequence of values for the dihedral angle from -1.45 radians to 0.0 radians with a step of 0.05
for AT in $(seq -1.45 0.05 0.0)
do
...
gmx_mpi mdrun -plumed eq1_plumed3.dat -deffnm eq1_nvt$AT -x eq1_traj$AT.xtc
...
# Run the simulation with GROMACS mdrun
gmx_mpi mdrun -plumed eq2_plumed3.dat -deffnm eq2_nvt$AT -x eq2_traj$AT.xtc
...
# Run the simulation with GROMACS mdrun
mpirun -np 1 gmx_mpi mdrun -plumed plumed3.dat -deffnm us$AT -x traj$AT.xtc
