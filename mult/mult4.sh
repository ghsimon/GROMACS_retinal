...
# Example usage: bash mult4.sh

# Loop over a sequence of values for the dihedral angle from 1.55 radians to 3.1 radians with a step of 0.05
for AT in $(seq 1.55 0.05 3.1)
do
...
gmx_mpi mdrun -plumed eq1_plumed4.dat -deffnm eq1_nvt$AT -x eq1_traj$AT.xtc
...
# Run the simulation with GROMACS mdrun
gmx_mpi mdrun -plumed eq2_plumed4.dat -deffnm eq2_nvt$AT -x eq2_traj$AT.xtc
...
# Run the simulation with GROMACS mdrun
mpirun -np 1 gmx_mpi mdrun -plumed plumed4.dat -deffnm us$AT -x traj$AT.xtc
